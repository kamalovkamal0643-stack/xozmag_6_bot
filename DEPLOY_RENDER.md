...
- **Бесплатный Render засыпает** через 15 минут без запросов. Первый ответ после сна занимает до минуты; Telegram повторяет доставку, сообщения не теряются.
- **Ежедневный отчёт в 20:00** не придёт, если сервис спит в это время. Варианты: тариф Starter (7 $/мес) или бесплатный пингер, например https://cron-job.org — запрос `https://xozmag-6-bot-api.onrender.com/api/health` каждые 10 минут.
